...
public class BookMyStay {
    ...
    public static void main(String[] args) {
            ...
            HashMap<String, String> reservationRoomMap = new HashMap<>();
...
// Print report
            ...
            try {
                if (!reservationMap.containsKey(cancelId)) {
                    throw new InvalidBookingException("Reservation not found!");
                }

                String roomType = reservationRoomMap.get(cancelId);

                rollbackStack.push(cancelId);

                roomInventory.put(roomType, roomInventory.get(roomType) + 1);

                reservationMap.remove(cancelId);
                reservationRoomMap.remove(cancelId);

                bookingHistory.add(cancelId + " CANCELLED");
